...
for i in proot dialog curl; do
	if [ -z "$(command -v ${i})" ]; then
		printf "\n\e[31mError: '${i}' is not installed.\e[0m\n\n"
		exit 1
	fi
done
version=$(dialog --title "Install Ubuntu" --inputbox "Enter the version code name:" 8 50 2>&1 > /dev/tty); clear
if [ -z "${version}" ]; then
	exit 1
fi
...
if ! proot --link2symlink tar -xf "${tarball}" --directory="${PREFIX}/share/${directory}" --exclude='dev' > /dev/null 2>&1; then
...
cat <<- EOF > "${PREFIX}/share/${directory}/etc/ld.so.preload"
	...
	EOF
if [ -d "${PREFIX}/share/${directory}/etc/profile.d" ]; then
	config_file="${PREFIX}/share/${directory}/etc/profile.d/config.sh"
else
	config_file="${PREFIX}/share/${directory}/etc/profile"
fi
cat <<- EOF >> "${config_file}"
	export LANG="C.UTF-8"
...
cat <<- EOF > "${PREFIX}/bin/start-${directory}"
	...
	unset LD_PRELOAD
	cmdline="proot"
	cmdline+=" --kernel-release=5.10.0"
	cmdline+=" --link2symlink"
	cmdline+=" --kill-on-exit"
	cmdline+=" --rootfs=\${PREFIX}/share/${directory}"
	cmdline+=" --root-id"
	cmdline+=" --bind=/dev"
	...
	cmdline+=" --bind=/sys"
	cmdline+=" --bind=/storage/self/primary:/sdcard"
	cmdline+=" --bind=/data/data/com.termux"
	cmdline+=" --bind=\${PREFIX}/share/${directory}/tmp:/dev/shm"
	if ! cat /proc/loadavg > /dev/null 2>&1; then
		cmdline+=" --bind=\${PREFIX}/share/${directory}/proc/.loadavg:/proc/loadavg"
	fi
	if ! cat /proc/stat > /dev/null 2>&1; then
		cmdline+=" --bind=\${PREFIX}/share/${directory}/proc/.stat:/proc/stat"
	fi
	if ! cat /proc/uptime > /dev/null 2>&1; then
		cmdline+=" --bind=\${PREFIX}/share/${directory}/proc/.uptime:/proc/uptime"
	fi
	if ! cat /proc/vmstat > /dev/null 2>&1; then
		cmdline+=" --bind=\${PREFIX}/share/${directory}/proc/.vmstat:/proc/vmstat"
	fi
	cmdline+=" --bind=\${PREFIX}/share/${directory}/proc/.model:/sys/firmware/devicetree/base/model"
	cmdline+=" --bind=\${PREFIX}/share/${directory}/proc/.version:/proc/version"
	cmdline+=" /usr/bin/env --ignore-environment"
	cmdline+=" TERM=\${TERM-xterm-256color}"
	cmdline+=" /bin/su --login"
	cmd="\$@"; [ -z "\$1" ] && \${cmdline} || \${cmdline} "\${cmd}"
	EOF
